...
next:
    ...
    {
        platform_printf("Info index:%d link:%d\r\n",conn->cur_ch_index, conn->event_info->link_num);
        btstack_push_user_msg(USER_TRIGGER_RX_CH_INFO, NULL, 0);
        return;
    }
    platform_printf("rev data ch:%d, link:%d\r\n", conn->cur_ch_index, conn->event_info->link_num);
    ...
    {
        platform_printf("re-try ch:%d, link:%d\n",conn->cur_ch_index, conn->event_info->link_num);
        goto next;
    }
    else
    {
        data_moni_data_cnt++;
        platform_printf("data rev return ch:%d link:%d\r\n", conn->cur_ch_index, conn->event_info->link_num);
        return;
    }
}
...
void on_rx_conn_info_done(struct ll_raw_packet *packet, void *user_data)
{
    ...
    for (i = 0; i < MAX_CONCURRENT_CONNECTIONS; i++)
    {
        ...
        if (conn_rx_infos[i].event_info->valid)
        {
            ...
            flag = 0;
            current_handle = i;
            platform_printf("info done link:%d, event:%d, offset:%d \r\n", i, conn_rx_infos[i].event_info->event_count, conn_rx_infos[i].event_info->time_offset);
            platform_printf("phy:%d, aa:%4x, itv:%d\r\n", conn_rx_infos[i].event_info->phy, conn_rx_infos[i].event_info->access_addr, conn_rx_infos[i].event_info->interval);
            rx_conn_event_infos[i].link_num = i;
            //platform_printf("aa:%4x chn:%d phy:%d\r\n ", conn_rx_infos[i].event_info->access_addr, conn_rx_infos[i].event_info->channels[0], conn_rx_infos[i].event_info->phy);
            if((conn_rx_infos[i].event_info->time_offset > 1000000)|| ((conn_rx_infos[i].event_info->time_offset < 0))||\
                (conn_rx_infos[i].event_info->event_count == rx_conn_event_infos_bak[i].event_count)||\
               (conn_rx_infos[i].event_info->interval > 2000000))
            {
                platform_printf("Wrong con info\r\n");
    ...
    platform_printf("info done trig rx\r\n");
              ...
              const void *data, int size, int rssi, void *user_data)
{
    ...
    if (status == 0)
    {
        platform_printf("DATA RSSI: @%d (%d) %3ddBm count:%d ch:%d  link:%d\r\n", id, index, rssi, conn->event_info->event_count,conn->cur_ch_index, conn->event_info->link_num);
        //platform_printf("index:%d\r\n",conn->cur_ch_index);
...
void on_rx_conn_data_done(struct ll_raw_packet *packet, void *user_data)
{
    ...
    if(pdu_num > 0)
        platform_printf("Rev PDU num: %d, ch:%d, link:%d\r\n",pdu_num, conn->cur_ch_index, conn->event_info->link_num);
    else
    {
        platform_printf("PDU 0,stop link:%d\r\n",id);
        conn->cur_ch_index = CHANNEL_INFO_NUM;
    }
    //ll_channel_monitor_check_each_pdu(packet, channel_monitor_pdu_visitor, user_data),
